...
id = '".$_GET['id']."'");
    ...
    <form action="proses_ubah_paket.php" method="post">
        <input type="hidden" name="id_paket" value= "<?=$dt_paket['id']?>">
        
        Jenis : 
        <?php 
        $arr_gender=array('kiloan'=>'Kiloan','selimut'=>'Selimut','bed_cover'=> 'Bed cover','kaos'=>'Kaos');
        ?>
        <select name="jenis" class="form-control">
            <option></option>
            <?php foreach ($arr_gender as $key_gender => $val_gender):
                if($key_gender==$dt_paket['jenis']){
                    $selek="selected";
                } else {
                    $selek="";
                }
