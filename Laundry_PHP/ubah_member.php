...
id = '".$_GET['id']."'");
    ...
    <form action="proses_ubah_member.php" method="post">
        <input type="hidden" name="id_member" value= "<?=$dt_member['id']?>">
        Nama Member :
  <input type="text" name="nama_member" value=   "<?=$dt_member['nama']?>" class="form-control">
        Alamat : 
...
<option value="<?=$key_gender?>" <?=$selek?>><?=$val_gender?></option>
        ...
        Telp :
  <input type="text" name="tlp_member" value=   "<?=$dt_member['tlp']?>" class="form-control">
        Kota:
